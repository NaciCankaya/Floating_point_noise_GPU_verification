...
comparing an eager inference run (uncompiled mode) vs. a compiled one (torch.compile). This is another model organism for
what "secret backend optimization" might be like, as in the CUDA version test. A problem arised when trying to extract
hidden states from a compiled model in decode inference, which was no issue in prefill: CUDA graphs. These immediately 
over-write hidden states after use. I could not do my experiments on hidden states for that reason, which is why I did these
only by comparing key vectors from attn layers. 

This worked so well, that I decided to use keys in every experiment from now
on, since they are just less invasive to extract in realistic inference monitoring and verification scenarios.
